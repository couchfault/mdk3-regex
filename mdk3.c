...
char use_deau[]="d   - Deauthentication / Disassociation Amok Mode\n"
		...
		"         Run test on MACs that match a regular expression (Blacklist regex mode)\n"
		"      -c [chan,chan,chan,...]\n"
	...
	case 0:
		...
		if (is_whitelisted(mac_sa)) goto newone;
	    	...
	    	if (wblist == 4) { // Using regex whitelist mode
	    		if (does_match_regex(mac_ta, (const char*)mac_regexp)) goto newone;
	    		if (does_match_regex(mac_sa, (const char*)mac_regexp)) goto newone;
	    	}
	    #endif
	    state = 1;
